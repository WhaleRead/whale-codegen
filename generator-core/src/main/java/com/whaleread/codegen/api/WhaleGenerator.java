...
import com.whaleread.codegen.internal.NullProgressCallback;
...
import static com.whaleread.codegen.internal.util.ClassloaderUtility.getCustomClassloader;
...
public class WhaleGenerator {
     ...
     */
    public WhaleGenerator(Configuration configuration, ShellCallback shellCallback,
    ...
    public void generate(ProgressCallback callback, Set<String> contextIds,
        ...
        for (Context context : contextsToRun) {
            context.generateFiles(callback, generatedFiles, warnings);
        }

        // now save the files
        if (writeFiles) {
            callback.saveStarted(generatedFiles.size());

            for (GeneratedFile wgf : generatedFiles) {
                projects.add(wgf.getTargetProject());
                writeGeneratedFile(wgf, callback);
            }
    ...
    private void writeGeneratedFile(GeneratedFile wgf, ProgressCallback callback) throws InterruptedException, IOException {
        File targetFile;
        String source;
        try {
            File directory = shellCallback.getDirectory(wgf
                    .getTargetProject(), wgf.getTargetPackage());
            targetFile = new File(directory, wgf.getFileName());
            if (targetFile.exists()) {
                if (shellCallback.isMergeSupported() && wgf.isMergeable()) {
                    source = wgf.merge(targetFile);
                    warnings.add(getString("Warning.101", //$NON-NLS-1$
                            targetFile.getAbsolutePath()));
                } else if (shellCallback.isOverwriteEnabled()) {
                    source = wgf.getFormattedContent();
                    warnings.add(getString("Warning.11", //$NON-NLS-1$
                            targetFile.getAbsolutePath()));
                } else {
                    source = wgf.getFormattedContent();
                    targetFile = getUniqueFileName(directory, wgf
                            .getFileName());
            ...
            writeFile(targetFile, source, wgf.getFileEncoding());
    ...
    /**
     * Returns the list of generated files after a call to one of the generate methods.
