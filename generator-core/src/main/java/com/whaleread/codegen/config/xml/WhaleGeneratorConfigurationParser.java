 ...
 * @author Jeff Butler
 */
public class WhaleGeneratorConfigurationParser {
    private Properties extraProperties;
    private Properties configurationProperties;

    public WhaleGeneratorConfigurationParser(Properties extraProperties) {
        super();
    ...
    private void parseContext(Configuration configuration, Node node) {
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            } else if ("builtInGenerator".equals(childNode.getNodeName())) { //$NON-NLS-1$
                parseBuiltInGenerator(context, childNode);
    ...
    protected void parseTable(Context context, Node node) {
        ...
        String javaTypeName = attributes.getProperty("javaTypeName"); //$NON-NLS-1$
        if (stringHasValue(javaTypeName)) {
            tc.setJavaTypeName(javaTypeName);
        }
...
//        }
    ...
    protected void parseBuiltInGenerator(Context context, Node node) {
        BuiltInGeneratorConfiguration configuration = new BuiltInGeneratorConfiguration();

        context
                .setBuiltInGeneratorConfiguration(configuration);
        ...
        if (stringHasValue(modelSubPackage)) {
            configuration.setModelSubPackage(modelSubPackage);
        }
        if (stringHasValue(dtoSubPackage)) {
            configuration.setDtoSubPackage(dtoSubPackage);
        }
        if (stringHasValue(daoSubPackage)) {
            configuration.setDaoSubPackage(daoSubPackage);
        }
    ...
    protected void parseConnectionFactory(Context context, Node node) {
        ...
        if (connectionFactoryConfiguration.getProperty("catalog") == null) {
            String path = connectionFactoryConfiguration.getProperty("path");
            String catalog = path.substring(path.lastIndexOf('/') + 1, path.lastIndexOf('.'));
            connectionFactoryConfiguration.addProperty("catalog", catalog);
        }
        String path = connectionFactoryConfiguration.getProperty("path");
        if (path != null) {
            if (!path.startsWith("classpath:") && !path.startsWith("file:") && !path.startsWith("/")) {
                connectionFactoryConfiguration.addProperty("path", extraProperties.getProperty("basedir") + System.getProperty("file.separator") + path);
            }
        }
    }
