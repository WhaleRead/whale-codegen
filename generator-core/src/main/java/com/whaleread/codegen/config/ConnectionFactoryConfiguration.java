...
public class ConnectionFactoryConfiguration extends TypedPropertyHolder {
    ...
    public void validate(List<String> errors) {
        ...
        if(getConfigurationType().equals("DDL")) {
            if (!StringUtility.stringHasValue(getProperty("parser"))) { //$NON-NLS-1$
                errors.add(getString("ValidationError.18", "connectionFactory", "parser")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            }
            if (!StringUtility.stringHasValue(getProperty("path"))) { //$NON-NLS-1$
                errors.add(getString("ValidationError.18", "connectionFactory", "path")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            }
        }
    }
