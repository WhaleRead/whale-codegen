...
public class Context extends PropertyHolder {
    ...
    public void validate(List<String> errors) {
        ...
        if (builtInGeneratorConfiguration == null) {
            errors.add(getString("ValidationError.8", id)); //$NON-NLS-1$
        } else {
            builtInGeneratorConfiguration.validate(errors, id);
        }
    ...
    public void initPlugins(List<String> warnings) {
    ...
    public void generateFiles(ProgressCallback callback,
                              List<GeneratedFile> generatedFiles, List<String> warnings)
            throws InterruptedException {
        initPlugins(warnings);
        ...
        if (introspectedTables != null) {
            for (IntrospectedTable introspectedTable : introspectedTables) {
                ...
                generatedFiles.addAll(introspectedTable
                        .getGeneratedFiles());

                generatedFiles.addAll(pluginAggregator
